"""
Evaluate Experiment 3 (Class Weighting) on test set.
"""
import torch
import numpy as np
...
def main():
    ...
    # Find best checkpoint from exp3
    checkpoint_dir = 'checkpoints/exp3_weighted'
    checkpoints = glob(f'{checkpoint_dir}/best_model_*.pth')
    
    if not checkpoints:
        print(f"No checkpoints found in {checkpoint_dir}")
        print("Training may still be in progress or not started.")
        return
    ...
    # Create dataloaders
    _, val_loader, test_loader = create_new_dataloaders(
        batch_size=32,
        num_workers=0
    )
    ...
    # Evaluate
    print("="*90)
    print("EXPERIMENT 3: CLASS WEIGHTING EVALUATION")
    print("="*90)
    ...
    results = {
        'experiment': 'Class Weighting (Custom Weights)',
        'checkpoint': best_checkpoint,
        'validation': val_metrics,
        'test': test_metrics
    }
    
    output_path = 'results/exp3_class_weighting.json'
