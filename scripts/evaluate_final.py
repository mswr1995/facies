...
Reports precision, recall, F1, PR-AUC for all classes.
"""
import torch
import numpy as np
...
def main():
    ...
    # Get the one with highest accuracy in filename
    ...
    print("\n" + "="*90)
    print("EVALUATING TEST SET (NEVER SEEN BEFORE)")
    print("="*90)
    ...
    print(f"\nOVERALL:")
    print(f"  {'Metric':<20} {'Validation':<15} {'Test':<15} {'Difference'}")
    print("-"*90)
    
    val_acc = val_results['overall']['accuracy']
    test_acc = test_results['overall']['accuracy']
    print(f"  {'Accuracy':<20} {val_acc:<15.4f} {test_acc:<15.4f} {(test_acc-val_acc):+.4f}")
    
    val_f1 = val_results['overall']['macro_f1']
    test_f1 = test_results['overall']['macro_f1']
    print(f"  {'Macro F1':<20} {val_f1:<15.4f} {test_f1:<15.4f} {(test_f1-val_f1):+.4f}")
    
    print(f"\nPER-CLASS F1:")
    print(f"  {'Class':<20} {'Validation':<15} {'Test':<15} {'Difference'}")
    print("-"*90)
    
    for class_name in class_names:
        val_f1 = val_results['per_class'][class_name]['f1']
        test_f1 = test_results['per_class'][class_name]['f1']
        print(f"  {class_name:<20} {val_f1:<15.4f} {test_f1:<15.4f} {(test_f1-val_f1):+.4f}")
    ...
    output_path = 'results/final_evaluation.json'
