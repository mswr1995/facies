...
Class imbalance is the main obstacle. In the dataset used here, peloids account for 87\% of annotated grains. Broken ooids, which are indicators of high-energy depositional events such as storms or strong tidal currents \citep{flugel2010}, represent approximately 1\%. A network trained on this distribution without modification will tend to predict peloid for most inputs, yielding high overall accuracy but poor minority-class performance \citep{johnson2019imbalance}.
...
Grains near a decision boundary can produce unstable predictions from a single deterministic forward pass \citep{shanmugam2021tta}. Each grain is therefore passed through the network six times under geometric transformations that preserve grain morphology: the original orientation, horizontal flip, vertical flip, and rotations by 90°, 180°, and 270°. The six sets of logits are averaged before the final decision:
\begin{equation}
...
Five of the six broken ooid test instances were correctly identified. Broken ooids are produced by mechanical fracture of intact ooid cortices during high-energy agitation, including storm events, strong tidal currents, or wave-driven reworking. Their presence in a thin section indicates a high-energy depositional environment, and their abundance relative to intact ooids can constrain the intensity and duration of such events within a carbonate succession.
...
Carbonate microfacies classification is a difficult machine learning problem not because the images are inherently complex, but because the class distribution is extreme and the minority classes are defined by subtle morphological signals that standard training procedures fail to recover. This paper proposes one approach to that problem: encode the geological taxonomy of carbonate grains in the network's branching structure, calibrate the loss function separately at each decision node to reflect the local class imbalance, oversample the most underrepresented class at the batch level, train in stages so that early majority-class learning does not dominate backbone updates, and average predictions over multiple orientations at inference time to stabilise boundary decisions.
