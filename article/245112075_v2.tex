...
On a held-out test set of 529 grains, the complete pipeline achieves 83.5\% Balanced Accuracy and 92.1\% Overall Accuracy. An ablation study identifies oversampling as the critical enabling step: without it, the hierarchical base recovers only 16.7\% of broken ooid instances. Oversampling alone raises this to 66.7\% and Balanced Accuracy to 80.0\%; the staged training curriculum brings broken ooid recall to 83.3\% while simultaneously improving intraclast recall from 66.7\% to 71.4\%. A flat ResNet-18 baseline achieves higher Balanced Accuracy on this test split (86.9\%), but its apparent advantage rests on a single class with only six test instances and is statistically unreliable. The hierarchical model's more robust gain is in intraclast recall: 71.4\% against 61.9\% for the flat baseline, on a sample of 21 instances with more stable uncertainty estimates.
\end{abstract}
...
An initial approach using 65 hand-crafted geometric and textural features with XGBoost classification confirmed this difficulty: the model achieved 80.9\% Balanced Accuracy but with ooid precision of 48.7\% and intraclast precision of 24.7\%, reflecting an inability to resolve the overlapping morphologies of the minority classes. This motivated a shift to a deep learning framework designed to handle the imbalance structurally.
...
The aims of this study are: (1) to develop a classification framework that reliably identifies all four carbonate grain types, including the rarest, from masked thin-section patches; and (2) to demonstrate through a systematic ablation study which components of the pipeline are necessary for minority-class recovery and which provide incremental improvement.
...
% NOTE FOR GALLOIS: Magnification and microscope model to be added here.
...
This formulation means the Stage 3 head --- responsible for the hardest and rarest distinction --- never receives gradient updates from the 2,300 peloid training samples.
...
Grains near a decision boundary can produce unstable predictions from a single deterministic forward pass. Each grain is therefore passed through the network six times, under six geometric transformations that preserve grain morphology: the original orientation, horizontal flip, vertical flip, and rotations by 90°, 180°, and 270°. The six sets of logits are averaged before the final decision:
\begin{equation}
...
Two baselines were evaluated on the same held-out test set.

The first is a traditional machine learning pipeline: XGBoost \citep{chen2016xgboost} trained on 65 hand-crafted geometric and textural features extracted from grain masks. Evaluated on a separate image-level hold-out split, it achieves 80.9\% Balanced Accuracy, with ooid precision 48.7\% and intraclast precision 24.7\%. Because this XGBoost evaluation used a different data partition from the annotation-level split used for all deep learning experiments, per-class recall figures are not directly comparable and are omitted from Table~\ref{tab:ablation}; the 80.9\% figure is reported here for context only.
...
\midrule
Flat ResNet-18, cross-entropy (baseline)$^\dag$ & 449 (97.6\%) & 37 (88.1\%) & 6 (100\%)  & 13 (61.9\%)  & 86.9\%       & \textbf{95.5\%} \\
...
+ 15$\times$ oversampling + TTA                  & \textbf{454 (98.7\%)} & 37 (88.1\%) & 4 (66.7\%) & 14 (66.7\%) & 80.0\%       & 96.2\% \\
+ Staged training + oversampling + TTA \textbf{(full model)} & 431 (93.7\%) & \textbf{36 (85.7\%)} & \textbf{5 (83.3\%)} & \textbf{15 (71.4\%)} & \textbf{83.5\%}  & 92.1\% \\
\bottomrule
\end{tabular}%
}
\end{table*}

The most striking result is the behaviour of the hierarchical network without oversampling. Despite having the same branching structure, stage-wise Focal Loss, and pre-trained backbone as the full model, this configuration recovers only 16.7\% of broken ooid instances (one grain out of six). This is barely above chance for a binary decision. Adding 15$\times$ targeted oversampling raises broken ooid recall to 66.7\% in a single step and lifts Balanced Accuracy from 63.3\% to 77.7\%.
...
Oversampling is the critical enabling step, not an optional refinement. Without it, the hierarchical base model recovers only 16.7\% of broken ooids despite having the branching architecture, stage-wise Focal Loss, and pre-trained backbone of the full model. Architecture alone cannot compensate for the near-absence of broken ooid gradient signal when fewer than 20 training examples are available. The 15$\times$ oversampling factor provides the Stage 3 head with sufficient training signal to learn a meaningful decision boundary.
...
The flat model's competitiveness on this particular test split does not indicate that it has learned a reliable representation of broken ooids. It reflects the difficulty of drawing conclusions from six test instances. We report both results because honest evaluation --- including cases where a simpler baseline is not clearly dominated --- is necessary for the field to progress.
...
% NOTE FOR GALLOIS: Please review and expand this subsection.
...
The dominant error in the confusion matrix --- 18 peloids classified as intraclast, and 3 intraclasts classified as peloid --- reflects a genuine ambiguity familiar to carbonate sedimentologists. Figure~\ref{fig:misclassifications} shows representative examples of both error directions from the test set.
...
The central empirical finding is that oversampling is the prerequisite for meaningful broken ooid detection, not an optional refinement. The hierarchical base without oversampling achieves only 16.7\% recall for broken ooids. Oversampling raises this to 66.7\% and Balanced Accuracy to 80.0\%. The staged training curriculum provides a more nuanced improvement: it does not dramatically change any single class's recall, but it produces the most consistently balanced performance across all four classes simultaneously, with no class falling below 71\% recall in the full model.
