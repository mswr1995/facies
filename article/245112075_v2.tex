...
Figure~\ref{fig:architecture} shows the complete architecture. The hierarchical architecture defines how information flows through the network; effective learning under severe class imbalance also requires a training procedure designed to preserve minority-class signal.
...
Table~\ref{tab:confusion} shows the confusion matrix for the full model, revealing the error types behind the per-class recall figures. Figure~\ref{fig:pr_curves} shows per-class Precision-Recall curves evaluated on the test set using continuous probability scores. The dominant source of confusion involves peloid and intraclast predictions, whereas broken ooids are identified correctly in five of the six test instances.
...
The dominant error in the confusion matrix (18 peloids classified as intraclast, and 3 intraclasts classified as peloids) reflects an ambiguity familiar to carbonate sedimentologists. Figure~\ref{fig:misclassifications} shows representative examples of both error directions from the test set.
...
Several limitations should be considered when interpreting the performance of the proposed framework.
